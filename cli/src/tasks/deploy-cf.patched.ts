...
async function syncWorkerSecrets(workerName: string) {
  ...
  if (secretKeys.length === 0) {
    console.log("â„¹ï¸ No worker secrets provided; skipping secret sync");
    return;
  }
...
export async function runCloudflareDeploy(target: "all" | "server" | "client" = "all", preview = false) {
  ...
  const cloudPasteApiBase = env("CLOUDPASTE_API_BASE", "");
  const cloudPastePublicBase = env("CLOUDPASTE_PUBLIC_BASE", "");
  const cloudPasteUploadPath = env("CLOUDPASTE_UPLOAD_PATH", "");
  ...
  Bun.write(
    ...
    stripIndent(`
      ...
      CLOUDPASTE_API_BASE = "${cloudPasteApiBase}"
      CLOUDPASTE_PUBLIC_BASE = "${cloudPastePublicBase}"
      CLOUDPASTE_UPLOAD_PATH = "${cloudPasteUploadPath}"
